...
interface MostCappedPlayersProps {
  mostCappedPlayers: MostCappedPlayer[];
}

const MostCappedPlayers = ({ mostCappedPlayers }: MostCappedPlayersProps) => {
  ...
  return (
    <div className="mb-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 px-6">
        {mostCappedPlayers.slice(0, displayCount).map((player, index) => (
          <Link
            key={index}
      ...
      {/* Show More Button */}
      {mostCappedPlayers.length > displayCount && (
...
export default MostCappedPlayers;
